...
exports.login = async (req, res) => {
  try {
    ...
    // ✅ Role validation — check selected role matches actual role in DB
    if (role) {
      const normalizedRole = role.toUpperCase();
      // ADMIN can log in as STAFF
      const allowedRoles = user.role === "ADMIN" ? ["ADMIN", "STAFF"] : [user.role];
      if (!allowedRoles.includes(normalizedRole)) {
        console.log(`❌ Role mismatch — user is ${user.role}, tried to login as ${normalizedRole}`);
